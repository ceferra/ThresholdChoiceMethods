...
########plot a set of vectors as a curve
plot_curves_testl<-function(curves,title="",ytit=LOSS,newplot=TRUE,points=TRUE,colors=c())
{

  if (newplot) plot(c(0,1),c(0,0),xlab= COSTRATIO,ylab=ytit, ylim=c(0,1), col="white",main=title,cex.main=0.8)
  title(title, cex.main = 1.4)#0.7
  ncurv =length(curves)
  leg.txt<-c()
  
  if (length(colors)>0) cols<-colors
  else cols<-c(2:(ncurv+1))
  ...
  for (j in 1:(ncurv))
  {
   print(cols[j])
    ...
    for (i in 1:(tam))
    {
      lines(c((i-1)/(tam-1),(i)/(tam-1)),c(vector[i],vector[i+1]),col=cols[j])
    }
  ...
  #leg.txt <- c(t1,t2,t3,t4,t5,t6)
  # print(leg.txt)
  y.leg <- c(2.5, 3, 2.1, 1.4, .7)
  cexv  <- c(1.2, 1, 4/5, 2/3, 1/2)
  #cols   <-c(2:(ncurv+1))
  xoff<-0.4 #xoffset of the legend
  
  j<-1
  print(leg.txt)
  legend(xoff, YLIM+0.03, leg.txt, pch = "___", col = cols, cex = 1)
#  legend(xoff, YLIM -0.01, leg.txt, pch = "___", col = cols, cex = 1)
...
################
##prints curves for complete noise (average) , we add an option to plot curves individually
plot_av_curves<-function(inp,trop,teop,sd,costlines,PLOTRD=TRUE,PLOTSD=TRUE,PLOTTR=TRUE,PLOTTE=TRUE){
  tam<-length(trop)
  ...
  dteop<-dteop/tam
  dsd<-dsd/tam
  #drd<-rep(1/(ncl-1),ncl) didnt work with ties
  #drd[1]<-drd[1]/2
  #drd[ncl]<-drd[ncl]/2
  drd<-compute_weights_rd_ties(costlines,inp)
  drd<-drd[which(drd!=0)] #filter 0
 # print(drd)
  #weigthed average
  ltrop<-c(0,0)
  lteop<-c(0,0)
...
if(PLOTSD)  lines(c(0,1),c(lsd[1],lsd[2]),col=2,lty=2,lwd=2)
if(PLOTTE)  lines(c(0,1),c(lteop[1],lteop[2]),col=3,lty=2,lwd=2)
if(PLOTRD)  lines(c(0,1),c(lrd[1],lrd[2]),col=4,lty=2,lwd=2)
if(PLOTTR)  lines(c(0,1),c(ltrop[1],ltrop[2]),col=5,lty=2,lwd=2)
 
  plot_av_curves<-list(atropt=(ltrop[1]+ltrop[2])/2,ateop=(lteop[1]+lteop[2])/2,ard=(lrd[1]+lrd[2])/2,asd=(lsd[1]+lsd[2])/2) 
}
...
exists_rep<-function(v,drd)
  {
  er<-FALSE
  for (i in 2:(length(v)))
    {
    if ((v[i-1]==v[i]) && (drd[i]!=0))
    {
      er<-TRUE
      break()
    }
...
compute_weights_rd_ties<-function(costlines,inp)
{
  ncl<-length(costlines)/2
  v<-rev(inp[,2])
  nclini<-length(v)+1 ### si no hi han repes
  drd<-rep(1/(nclini-1),nclini)
  drd[1]<-drd[1]/2
  drd[nclini]<-drd[nclini]/2
  i<-1
 # print(drd)
#  print(v)
  while (i<(length(v)-1))
  { k<-1
    while (v[i]==v[i+k]&&(i+k)<(1+length(v)) ) k<-k+1 # we count repetitions
    if(k>1)# there are repetitions, we distribute their weights and assign 0
    {
     #print(k)
      acum<-sum(drd[(i+1):(i+k-1)])
     # print(drd[(i+1):(i+k-1)])
      drd[(i+1):(i+k-1)]<-0
      drd[i]<-drd[i]+acum/2
      drd[i+k]<-drd[i+k]+acum/2
    }
    i<-i+k
